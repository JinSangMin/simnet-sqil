...
if __name__ == '__main__':
    ...
    end_id = train_dataset.get_scene_indices(min(args.end, last_id))[-1]
    print(start_id, end_id)

    last_id = train_dataset[-1]['scene_index']
    scene_idx_start = max(args.start, 0)
    scene_idx_end = min(args.end, last_id)

    dict = {}
    for scene_idx_ in range(scene_idx_start, scene_idx_end + 1):
        current_dict = {}
        image_indices = train_dataset.get_scene_indices(scene_idx_)
        for i in image_indices:
            ...
            # If frame is not sufficient, move on to next image
            ...
            current_dict[i] = {}
            if ex['track_id'] not in current_dict[i].keys():
                current_dict[i][ex['track_id']] = {}
            current_dict[i][ex['track_id']][ex['frame_index']] = {'state_id': i,
                                                                  'action': (curr_acc, curr_steer)}

            if ex['track_id'] not in current_dict.keys():
                current_dict[ex['track_id']] = {}
            current_dict[ex['track_id']][ex['frame_index']] = {'state_id': i,
                                                               'action': (curr_acc, curr_steer)}

        for track_id, track_info in current_dict.items():
            frame_list = list(track_info.keys())
            for k in range(len(frame_list)):
                if k+1<len(frame_list) and frame_list[k+1] == frame_list[k]+1:
                    state_id = track_info[frame_list[k]]['state_id']
                    action = track_info[frame_list[k]]['action']
                    next_state_id = track_info[frame_list[k+1]]['state_id']
                    dict[state_id] = {'scene_index': scene_idx_,
                                       'track_id': track_id,
                                       'frame_index': frame_list[k],
