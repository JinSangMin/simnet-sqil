...
class L5Env(gym.Env):
    ...
    def __init__(self, env_config_path: Optional[str] = None, dmg: Optional[LocalDataManager] = None,
                 ...
                 randomize_start: bool = True, simnet_model_path: Optional[str] = None,
                 sqil: tuple = None) -> None:
        """Constructor method
        """
        ...
        if sqil is not None:
            self.num_steer = sqil[0]
            self.num_accel = sqil[1]
        ...
        # if sqil is not None:
        #     self.dataset = AgentDataset(cfg, dataset_zarr, rasterizer) # same with simnet
        ...
        if sqil is not None:
            # Discrete Action Space: gym.spaces.Box (X, Y, Yaw * number of future states)
            actionsp = {
                'steer': spaces.Discrete(self.num_steer),
                'accel': spaces.Discrete(self.num_accel)
                }
            self.action_space = spaces.Dict(actionsp)

        # Observation Space: gym.spaces.Dict (image: [n_channels, raster_size, raster_size])
        obs_shape = (n_channels, raster_size, raster_size)
        
        # if sqil is not None:
        #     obs_shape = (n_channels - 2, raster_size, raster_size)
